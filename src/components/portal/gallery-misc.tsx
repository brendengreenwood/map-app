...
function DatePicker() {
  ...
  return (
    <Popover>
      <PopoverTrigger render={<Button variant="outline" className={cn("w-[260px] justify-start gap-2 font-normal", !date && "text-muted-foreground")} > <CalendarIcon className="size-4" /> {date ? format(date, "PPP") : "Pick a date"} </Button>} />
        ...
        <Demo className="items-start">
          <Collapsible open={open} onOpenChange={setOpen} className="w-full max-w-xs space-y-2">
            <div className="flex items-center justify-between rounded-md border bg-card px-4 py-2.5">
              ...
              <CollapsibleTrigger render={<Button variant="ghost" size="icon" className="size-7"> <ChevronsUpDown /> </Button>} />
            </div>
