...
import { CARTO_URLS, sproutTransformStyle } from '@/lib/map-styles';
...
export function useMap(containerRef: React.RefObject<HTMLDivElement | null>, theme: ResolvedTheme = 'light', onReady?: () => void) {
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      transformStyle: sproutTransformStyle(theme),
    });
  ...
  const setTheme = useCallback((newTheme: ResolvedTheme) => {
    ...
    map.setStyle(cartoUrl, { transformStyle: sproutTransformStyle(newTheme) });
    map.once('style.load', () => {
